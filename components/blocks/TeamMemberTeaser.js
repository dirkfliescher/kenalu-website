...
export default function TeamMemberTeaser({ member }) {
  ...
  const calendly = content.team_member_calendly_link;

  return (
    <div className="team-card">
      <Link href={`/about/${slug}`} className="team-card-link">
      ...
      {calendly && (
        <a
          ...
          className="team-card-cta btn btn-sm btn-ghost"
        >
          Gespräch mit {content.team_member_name?.split(' ')[0]} →
        </a>
      )}
    </div>
  );
